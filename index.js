...
fenomenler[6].profile = 'Justin Bieber' /*atama yapıldı*/
...
function profilListesi(dizi) {
  let yeniDizi = [...dizi] /*spread objeyi kkopyalasaydık süslü parantez olacaktı*/
...
function fenomenEkle(arr,n,pr,f,po,pl) {
  let tempArr = [...arr]
  let yeniFenomen ={
      number : n,
      profile : pr,
  ...
  tempArr.push(yeniFenomen);
  return tempArr
}
//console.log(fenomenEkle)
...
//console.log("görev 8 posts",fenomenGonderimSayisi(fenomenler,"Will Smith"));
...
function platformaGoreCokGonderiYapanFenomen(feno,platform){
  ...
  for(let i=0;i<feno.length;i++){
    if(feno[i].platform==platform && feno[i].posts>maksGonderi){
     maksGonderi= feno[i].posts 
     maksGonderen=feno[i].profile
    }
